...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        messages = [{"role": "system", "content": "Bạn là trợ lý AI hữu ích."}] + history + [{"role": "user", "content": user_msg}]
...
def run_assistant(
    ...
    while True:
        ...
        turn_cost = estimate_cost(user_msg, reply)["total_cost"]
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    """
    lines = []
    lines.append(f"{'Prompt':<40} | {'GPT-4o Response':<40} | {'Mini Response':<40} | {'GPT-4o Latency':<15} | {'Mini Latency':<15}")
    lines.append("-" * 155)
    for r in results:
        prompt = r.get("prompt", "")[:40]
        gpt4o_resp = r.get("gpt4o_response", "")[:40]
        mini_resp = r.get("mini_response", "")[:40]
        gpt4o_lat = f"{r.get('gpt4o_latency', 0):.3f}"
        mini_lat = f"{r.get('mini_latency', 0):.3f}"
        lines.append(f"{prompt:<40} | {gpt4o_resp:<40} | {mini_resp:<40} | {gpt4o_lat:<15} | {mini_lat:<15}")
    return "\n".join(lines)
